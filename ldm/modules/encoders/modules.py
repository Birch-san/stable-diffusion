...
class CLIPEmbedding(NamedTuple):
...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text) -> CLIPEmbedding:
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device)
        attention_mask=(tokens != self.tokenizer.pad_token_id)
        outputs = self.transformer(input_ids=tokens, attention_mask=attention_mask.long())

        return CLIPEmbedding(outputs.last_hidden_state, attention_mask)
